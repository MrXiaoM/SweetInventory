...
@AutoRegister(priority = 995, requirePlugins = "SX-Item")
public class SXItemMatcher extends AbstractModule implements ItemMatcher.Provider {
    public SXItemMatcher(SweetInventory plugin) {
        super(plugin);
    ...
    public ItemMatcher parse(@NotNull ConfigurationSection config) {
        Pair<String, AbstractStringMatcher.Type> pair = AbstractStringMatcher.Type.parse("sx-item", config::contains);
        if (pair != null) {
