...
@AutoRegister(priority = 995, requirePlugins = "NeigeItems")
public class NeigeItemsMatcher extends AbstractModule implements ItemMatcher.Provider {
    public NeigeItemsMatcher(SweetInventory plugin) {
        super(plugin);
    ...
    public ItemMatcher parse(@NotNull ConfigurationSection config) {
        Pair<String, AbstractStringMatcher.Type> pair = AbstractStringMatcher.Type.parse("neige-items", config::contains);
        if (pair != null) {
